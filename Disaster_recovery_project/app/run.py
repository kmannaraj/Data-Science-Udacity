...
def index():
    ...
    # Getting message count by genre and related status  
    genre_related_counts = df.groupby('genre').count()['message']
    ...
    category_props = df.drop(['id', 'message', 'original', 'genre'], axis = 1).sum()/len(df)
    category_props = category_props.sort_values(ascending = False)
    category_names = list(category_props.index)
